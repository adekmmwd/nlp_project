...
UNTRAINED_DISPLAY_NAME = "Untrained Legal-BERT"
FINE_TUNED_MODEL_CANDIDATES = [
    (
        "legal_bert_109m_trained",
        os.path.join(MODELS_DIR, "legal_bert_109m_trained_best.pt"),
    ),
    (
        "legal_bert_57m_trained",
        os.path.join(MODELS_DIR, "legal_bert_57m_trained_best.pt"),
    ),
    ("legal_bert_trained_legacy", os.path.join(MODELS_DIR, "bert_best.pt")),
]
...
print("\nBaseline note:")
print("  legal-bert-base-uncased has no trained risk-classification head.")
print("  This script evaluates its randomly initialized classifier head.")
print("  Use it as an untrained Legal-BERT sanity baseline, not as a legal-risk model.")
...
# ============================================================
print("\nEvaluating untrained legal-bert-base-uncased classifier...")
...
untrained_preds, test_labels, untrained_probs = evaluate_model(
    untrained_model, test_loader, UNTRAINED_DISPLAY_NAME
)
...
untrained_cm_path = save_confusion_matrix(
    ...
    test_labels,
    untrained_preds,
    f"{UNTRAINED_DISPLAY_NAME} - Confusion Matrix",
)
untrained_prob_path = save_probability_histogram(
    UNTRAINED_PREFIX,
    test_labels,
    untrained_probs,
    f"{UNTRAINED_DISPLAY_NAME} - Risk Probability Distribution",
)
untrained_metrics_plot_path = save_metrics_bar_chart(
    [(UNTRAINED_DISPLAY_NAME, untrained_metrics)],
    f"{UNTRAINED_PREFIX}_metrics.png",
    f"{UNTRAINED_DISPLAY_NAME} Metrics",
)
untrained_roc_path = save_roc_curve(
    UNTRAINED_PREFIX,
    test_labels,
    untrained_probs,
    f"{UNTRAINED_DISPLAY_NAME} - ROC Curve",
)

print_metrics_table([(UNTRAINED_DISPLAY_NAME, untrained_metrics)])
print("\nClassification report - untrained Legal-BERT")
print(
    classification_report(
        test_labels,
        untrained_preds,
...
if fine_tuned_checkpoint is not None:
    ...
    comparison_metrics_plot_path = save_metrics_bar_chart(
        ...
        f"{UNTRAINED_DISPLAY_NAME} vs {fine_tuned_title} Metrics",
    )
    comparison_cm_path = save_confusion_matrix_grid(
        [
            (UNTRAINED_DISPLAY_NAME, test_labels, untrained_preds),
    ...
    comparison_roc_path = save_roc_curve_comparison(
        [
            (UNTRAINED_DISPLAY_NAME, test_labels, untrained_probs),
            (fine_tuned_title, fine_labels, fine_probs),
        ],
        f"{UNTRAINED_PREFIX}_vs_{fine_tuned_prefix}_roc_curve.png",
        f"{UNTRAINED_DISPLAY_NAME} vs {fine_tuned_title} ROC Curve Comparison",
    )
